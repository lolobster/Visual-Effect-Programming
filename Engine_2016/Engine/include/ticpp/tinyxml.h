...
#define TIXML_USE_STL
	...
	typedef TiXmlInStream TIXML_ISTREAM;
	typedef TiXmlOutStream TIXML_OSTREAM;
	typedef eastl::string TIXML_STRING;
#endif
#else
	#include "tinystr.h"
...
public:
	...
	*/
	virtual void Print( core::FileStream* cfile, int depth ) const = 0;
...
public:
	...
	*/
    ...
    #ifdef TIXML_USE_STL
	const TiXmlNode* FirstChild( const TIXML_STRING& _value ) const	{	return FirstChild (_value.c_str ());	}	///< STL std::string form.
	TiXmlNode* FirstChild( const TIXML_STRING& _value )				{	return FirstChild (_value.c_str ());	}	///< STL std::string form.
	const TiXmlNode* LastChild( const TIXML_STRING& _value ) const	{	return LastChild (_value.c_str ());	}	///< STL std::string form.
	TiXmlNode* LastChild( const TIXML_STRING& _value )				{	return LastChild (_value.c_str ());	}	///< STL std::string form.
	#endif
	...
	*/
    ...
    #ifdef TIXML_USE_STL
	const TiXmlNode* IterateChildren( const TIXML_STRING& _value, const TiXmlNode* previous ) const	{	return IterateChildren (_value.c_str (), previous);	}	///< STL std::string form.
	TiXmlNode* IterateChildren( const TIXML_STRING& _value, const TiXmlNode* previous ) {	return IterateChildren (_value.c_str (), previous);	}	///< STL std::string form.
	#endif
	...
	*/
    ...
    #ifdef TIXML_USE_STL
	const TiXmlNode* PreviousSibling( const TIXML_STRING& _value ) const	{	return PreviousSibling (_value.c_str ());	}	///< STL std::string form.
	TiXmlNode* PreviousSibling( const TIXML_STRING& _value ) 			{	return PreviousSibling (_value.c_str ());	}	///< STL std::string form.
	const TiXmlNode* NextSibling( const TIXML_STRING& _value) const		{	return NextSibling (_value.c_str ());	}	///< STL std::string form.
	TiXmlNode* NextSibling( const TIXML_STRING& _value) 					{	return NextSibling (_value.c_str ());	}	///< STL std::string form.
	#endif
	...
	*/
    ...
    #ifdef TIXML_USE_STL
	const TiXmlElement* NextSiblingElement( const TIXML_STRING& _value) const	{	return NextSiblingElement (_value.c_str ());	}	///< STL std::string form.
	TiXmlElement* NextSiblingElement( const TIXML_STRING& _value)				{	return NextSiblingElement (_value.c_str ());	}	///< STL std::string form.
	#endif
    ...
    #ifdef TIXML_USE_STL
	const TiXmlElement* FirstChildElement( const TIXML_STRING& _value ) const	{	return FirstChildElement (_value.c_str ());	}	///< STL std::string form.
	TiXmlElement* FirstChildElement( const TIXML_STRING& _value )				{	return FirstChildElement (_value.c_str ());	}	///< STL std::string form.
	#endif
...
public:
	...
	*/
	...
	virtual void Print(core::FileStream* cfile, int depth) const {
		Print( cfile, depth, 0 );
	}
	void Print(core::FileStream* cfile, int depth, TIXML_STRING* str) const;
...
public:
	...
	*/
	template< typename T > int QueryValueAttribute( const TIXML_STRING& name, T* outValue ) const
	{
		...
		TIXML_ISTREAM sstream( node->ValueStr() );
		sstream >> *outValue;
		if ( !sstream.fail() )
...
public:
	...
	virtual void Print(core::FileStream* cfile, int depth, TIXML_STRING* str) const;
	virtual void Print(core::FileStream* cfile, int depth) const {
		Print( cfile, depth, 0 );
	}
...
#ifdef TIXML_USE_STL
	/// Constructor.
	TiXmlStylesheetReference(	const TIXML_STRING& _type,
								const TIXML_STRING& _href );
#endif
...
public:
	...
	*/
	bool LoadFile(core::FileStream*, TiXmlEncoding encoding = TIXML_DEFAULT_ENCODING);
	/// Save a file using the given FILE*. Returns true if successful.
	bool SaveFile(core::FileStream*) const;

	#ifdef TIXML_USE_STL
	bool LoadFile( const TIXML_STRING& filename, TiXmlEncoding encoding = TIXML_DEFAULT_ENCODING )			///< STL std::string version.
	{
...
//		return ( f.buffer && SaveFile( f.buffer ));
	...
	*/
	...
	virtual void Print(core::FileStream* cfile, int depth = 0) const;
	// [internal use]
