...
void TiXmlElement::SetAttribute( const TIXML_STRING& name, int val )
{
   TIXML_OSTREAM oss;
   oss << val;
   SetAttribute( name.c_str(), oss.str().c_str() );
}
#endif
...
void TiXmlElement::Print(core::FileStream* cfile, int depth) const
{
	int i;
	assert( cfile );
	for ( i=0; i<depth; i++ ) {
		const char* str = "    ";
		cfile->write(str,strlen(str));
	}

	{
		eastl::string str;
		str.sprintf("<%s", value.c_str() );
...
bool TiXmlDocument::LoadFile( const char* _filename, TiXmlEncoding encoding )
{
	...
	core::Ref<core::FileStream> file = new core::FileStream(value.c_str(), core::FileStream::READ_ONLY);//TiXmlFOpen( value.c_str (), "rb" );
...
bool TiXmlDocument::LoadFile(core::FileStream* file, TiXmlEncoding encoding)
{
	if ( !file )
	{
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
	...
	*/
	...
	char* buf = new char[ file->available()+1 ];
	buf[0] = 0;

	if ( file->read( buf, file->available() ) != length ) {
		delete [] buf;
		SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
		return false;
	}
...
bool TiXmlDocument::SaveFile( const char * filename ) const
{
	...
	core::Ref<core::FileStream> file = new core::FileStream(filename, core::FileStream::READ_WRITE);
	if ( file )
	{
...
bool TiXmlDocument::SaveFile(core::FileStream* file) const
{
	if ( useMicrosoftBOM )
	{
		...
		file->write( &TIXML_UTF_LEAD_0, sizeof(TIXML_UTF_LEAD_0) );
		file->write( &TIXML_UTF_LEAD_1, sizeof(TIXML_UTF_LEAD_1) );
		file->write( &TIXML_UTF_LEAD_2, sizeof(TIXML_UTF_LEAD_2) );
	}
...
void TiXmlDocument::Print(core::FileStream* cfile, int depth) const
{
	assert( cfile );
	for ( const TiXmlNode* node=FirstChild(); node; node=node->NextSibling() )
	{
...
void TiXmlAttribute::Print(core::FileStream* cfile, int /*depth*/, TIXML_STRING* str) const
{
	TIXML_STRING n, v;
	...
	if (value.find ('\"') == TIXML_STRING::npos) {
		if ( cfile ) {
			eastl::string s;
			s.sprintf("%s=\"%s\"", n.c_str(), v.c_str());
			cfile->write(s.c_str(),s.length());
		}
		if ( str ) {
			(*str) += n; (*str) += "=\""; (*str) += v; (*str) += "\"";
		}
...
void TiXmlComment::Print(core::FileStream* cfile, int depth) const
{
	assert( cfile );
...
void TiXmlText::Print(core::FileStream* cfile, int depth) const
{
	assert( cfile );
	if ( cdata )
	{
	...
	else
	{
		...
		cfile->write(buffer.c_str(),buffer.length());
	}
}
...
void TiXmlDeclaration::Print(core::FileStream* cfile, int /*depth*/, TIXML_STRING* str) const
{
	if ( cfile ) 
	{
		const char* s = "<?xml ";
	...
	if ( !version.empty() ) 
	{
		...
		{
			eastl::string s;
			s.sprintf("version=\"%s\" ", version.c_str ());
			cfile->write(s.c_str(),s.length());
		}
	...
	if ( !encoding.empty() )
	{
		...
		{
			eastl::string s;
			s.sprintf("encoding=\"%s\" ", encoding.c_str ());
			cfile->write(s.c_str(),s.length());
		}
	...
	if ( !standalone.empty() )
	{
		...
		{
			eastl::string s;
			s.sprintf("standalone=\"%s\" ", standalone.c_str ());
			cfile->write(s.c_str(),s.length());
		}
...
#ifdef TIXML_USE_STL
TiXmlStylesheetReference::TiXmlStylesheetReference(	const TIXML_STRING& _type,
...
void TiXmlStylesheetReference::Print(core::FileStream* cfile, int /*depth*/, TIXML_STRING* str) const
{
	if ( cfile ) 
	{
		const char* s = "<?xml-stylesheet ";
		cfile->write(s,strlen(s));
	}
...
void TiXmlUnknown::Print(core::FileStream* cfile, int depth) const
{
	for ( int i=0; i<depth; i++ )
	{
		const char* s = "    ";
	...
	cfile->write(value.c_str(),value.length());
}
